      ...
      {cards.map((card) => {
        ...
        return (
          <div
            ...
            className={`cursor-pointer rounded-lg p-3 transition-all duration-200 relative overflow-hidden group ${
              card.isActive
          ...
          >
            ...
            <div className="flex items-start justify-between mb-2">
              ...
              <span className={`text-[9px] font-bold px-2 py-0.2 rounded-full border border-white/80 shadow-xs ${card.badgeBg}`}>
                {card.status}
              </span>
            </div>
            ...
            <div className="space-y-0.5">
              ...
              <h3 className="text-sm font-bold text-slate-900 group-hover:text-[#7a8315] transition-colors flex items-center justify-between">
                {card.title}
                {card.isDone && <CheckCircle2 className="w-3 h-3 text-emerald-600 inline" />}
                {!card.isDone && !card.isActive && <Lock className="w-3 h-3 text-slate-400 inline" />}
              </h3>
            </div>
            ...
            <div className="mt-2 pt-2 border-t border-slate-200/50 flex items-center justify-between">
              ...
              {card.isActive && (
                <div className="w-1.5 h-1.5 rounded-full bg-[#96a01d] animate-ping" />
              )}
            </div>
          </div>
        );
