...
export default function DecisionCard({ project, onRework, onRelease, showToast }) {
  ...
  return (
    <div className="glass-strong rounded-xl p-4 sm:p-5 border border-white/90 shadow-md relative overflow-hidden backdrop-blur-md mb-3 w-full text-center">
      ...
      <div className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-emerald-100 text-emerald-800 text-[10px] font-bold border border-emerald-200 mb-1.5">
        Testing Completed ✓
      </div>
      ...
      <p className="text-[11px] text-slate-600 font-medium max-w-md mx-auto mb-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3.5 items-stretch mb-3 text-left max-w-4xl mx-auto">
        ...
        <div className="glass-card rounded-xl p-3.5 border border-white/90 flex flex-col justify-between hover:border-slate-300 transition-all">
          ...
          <button
            ...
            className="w-full py-2.5 px-3 rounded-lg bg-slate-100 hover:bg-slate-200/80 text-slate-800 font-bold text-xs border border-slate-300/80 shadow-xs active:scale-95 transition-all flex items-center justify-center gap-1 min-h-[44px]"
          >
        ...
        <div className="glass-theme rounded-xl p-3.5 border-2 border-[#96a01d]/70 flex flex-col justify-between relative shadow-sm shadow-[#96a01d]/20 scale-[1.005] transform transition-all hover:scale-[1.01]">
          ...
          <button
            onClick={onRelease}
            className="w-full py-2.5 px-3 rounded-lg bg-gradient-to-r from-[#96a01d] via-[#a8b422] to-[#7a8315] text-slate-950 font-extrabold text-xs shadow-xs shadow-[#96a01d]/35 hover:scale-[1.005] active:scale-95 transition-all duration-200 flex items-center justify-center gap-1 group min-h-[44px]"
          >
      ...
      {/* Rework Modal Dialog */}
      {showReworkModal && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-3 bg-slate-900/30 backdrop-blur-md animate-in fade-in">
          <div className="glass-strong rounded-t-2xl sm:rounded-xl p-4 border border-white/90 max-w-sm w-full shadow-lg text-left">
            <h3 className="text-sm font-bold text-slate-900 mb-1 flex items-center gap-1">
              <AlertTriangle className="w-3.5 h-3.5 text-[#96a01d]" />
            ...
            <textarea
              ...
              className="w-full h-18 glass-input rounded-lg p-2 text-xs mb-2 text-slate-800"
            />
            <div className="flex items-center justify-end gap-1.5 pb-2 sm:pb-0">
              <button
                onClick={() => setShowReworkModal(false)}
                className="px-3 py-2 rounded-md text-xs font-bold text-slate-600 hover:bg-white/60 min-h-[40px]"
              >
                Cancel
              </button>
              <button
                ...
                className="px-4 py-2 rounded-md bg-slate-950 text-[#96a01d] font-extrabold text-xs shadow-xs hover:bg-slate-900 min-h-[40px]"
              >
                Confirm Rework
              </button>
            </div>
